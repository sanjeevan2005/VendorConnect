...
VENDOR_PHONE_OVERRIDE = os.getenv("VENDOR_PHONE_OVERRIDE", "").strip()
...
def _normalize_phone(v: Any) -> Optional[str]:
    phone = _coerce_str(v)
    if not phone:
        return None
...
def _vendor_phone(vendor: dict) -> Optional[str]:
    if VENDOR_PHONE_OVERRIDE:
        return _normalize_phone(VENDOR_PHONE_OVERRIDE)
    contact = vendor.get("contact") or {}
    return _normalize_phone(contact.get("phone") or vendor.get("phone"))
...
def parse_rfq(req: ParseRFQRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"RFQ parsing failed: {e}")
...
def call_vendor(req: CallVendorRequest) -> CallResponse:
    ...
    if not vendor_phone:
        raise HTTPException(status_code=400, detail="Vendor phone is missing or not E.164. Set VENDOR_PHONE_OVERRIDE for demo calls.")
...
def _populate_dummy_vendor(row: dict, rfq: DiscoverVendorsRequest) -> dict:
    ...
    if status in ("quoted", "qualified", "responded"):
        ...
        lead_weeks = rng.choice([3, 4, 5, 6, 8, 10, 12]) if rfq.timeline_weeks is None else rng.randint(max(1, rfq.timeline_weeks - 2), rfq.timeline_weeks + 4)
...
def discover_vendors(req: DiscoverVendorsRequest):
    if not CRUST_DATA_API_KEY:
        raise HTTPException(status_code=500, detail="CRUST_DATA_API_KEY is not set")
    sb = supabase_client()
    if sb is None:
    ...
    try:
        with httpx.Client() as client:
            ...
            for idx, c in enumerate(companies):
                ...
                if idx == 0:
                    # Optional demo override keeps test calls explicit and out of production defaults.
                    if VENDOR_PHONE_OVERRIDE:
                        contact = dict(row.get("contact") or {})
                        contact["phone"] = VENDOR_PHONE_OVERRIDE
                        row["contact"] = contact
                else:
            ...
            if vendors_out and VAPI_ASSISTANT_ID:
                ...
                try:
                    ...
                    vendor_phone = _vendor_phone(vendor0)
                    if not vendor_phone:
                        raise ValueError("vendor phone missing")
                    trigger_call(
                        assistant_id=VAPI_ASSISTANT_ID,
                        vendor_phone=vendor_phone,
                        variables=variables,
        ...
        raise HTTPException(status_code=502, detail=f"Crust Data {e.response.status_code}: {e.response.text[:200]}")
    except Exception as e:
